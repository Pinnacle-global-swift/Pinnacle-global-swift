...
import { Shield, Upload, CheckCircle2, Check, Loader2, XCircle } from "lucide-react"
...
const formSchema = z.object({
  ...
  idFrontImage: z.object({
    file: z.instanceof(File, { message: "Front ID image is required" }).nullable(),
    url: z.string().nullable(),
  }).refine((data) => data.file !== null, {
    message: "Front ID image is required",
  }),
  idBackImage: z.object({
    file: z.instanceof(File, { message: "Back ID image is required" }).nullable(),
    url: z.string().nullable(),
  }).refine((data) => data.file !== null, {
    message: "Back ID image is required",
  }),
  proofOfAddressImage: z.object({
    file: z.instanceof(File, { message: "Proof of address document is required" }).nullable(),
    url: z.string().nullable(),
  }).refine((data) => data.file !== null, {
...
export default function KYCPage() {
  ...
  const [frontImage, setFrontImage] = useState<{ file: File | null; url: string | null }>({ file: null, url: null })
  const [backImage, setBackImage] = useState<{ file: File | null; url: string | null }>({ file: null, url: null })
  const [proofOfAddressImage, setProofOfAddressImage] = useState<{ file: File | null; url: string | null }>({
    file: null,
    url: null,
  })
    ...
    try {
      ...
      Object.entries(values).forEach(([key, value]) => {
        if (key === "idFrontImage") {
          ...
          formData.append("proofOfAddress", values.proofOfAddressImage.file as File)
        } else {
  ...
  const handleProofOfAddressChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (file) {
      const imageData = {
        file,
  ...
  if (kycstatus.status === "approved") {
    return (
      ...
      >
        ...
        <motion.p className="mt-4 text-gray-300 text-center max-w-xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        Your KYC application has been approved. You can now proceed to use our services.
        </motion.p>
      </motion.div>
    )
  }
  ...
  if (kycstatus?.status === "processing") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px]">
        ...
        <p className="text-gray-600 mt-2">Your application is currently under review.</p>
      </div>
    )
  }
    ...
    <div className="container max-w-3xl mx-auto py-10 px-4 sm:px-6">
      ...
      >
        ...
        <CardContent className="p-8">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8" ref={animationParent}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-100">dateOfBirth</FormLabel>
                      <FormControl>
                        <Input
                          type="date"
                          className={cn(
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-100">Nationality</FormLabel>
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent className={cn("bg-gray-950/50 border-white/20 text-gray-100", "rounded-xl")}>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-100">Identity Type</FormLabel>
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl>
                          <SelectTrigger
                            className={cn(
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-100">Identity Number</FormLabel>
                      <FormControl>
                        <Input
                          className={cn(
                            "bg-gray-950/50 border-white/20 text-gray-100",
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-100">Residential Address</FormLabel>
                      <FormControl>
                        <Input
                          className={cn(
                            "bg-gray-950/50 border-white/20 text-gray-100",
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <label className="block text-gray-100 font-medium">Id Front Image</label>
                  <div className="group relative">
                    ...
                    >
                      <div className="flex flex-col items-center justify-center gap-4">
                        ...
                        <p className="text-center text-gray-400">Upload ID Front Image</p>
                      </div>
                      <input
                        type="file"
                  ...
                  {frontImage.url && (
                    <div className="mt-4">
                      <p className="text-sm text-gray-400 mb-2">Front Image Preview:</p>
                      <img
                ...
                <div className="space-y-4">
                  <label className="block text-gray-100 font-medium">Id Back Image</label>
                  <div className="group relative">
                    <div
                      className={cn(
                        "border-2 border-dashed border-white/20 rounded-xl p-8",
                    ...
                    >
                      <div className="flex flex-col items-center justify-center gap-4">
                        <Upload className="w-10 h-10 text-blue-400" />
                        <p className="text-center text-gray-400">Upload ID Back Image</p>
                      </div>
                      <input
                        type="file"
                        className="absolute inset-0 opacity-0 cursor-pointer"
                  ...
                  {backImage.url && (
                    <div className="mt-4">
                      <p className="text-sm text-gray-400 mb-2">Back Image Preview:</p>
                      <img
                ...
                <div className="space-y-4">
                  <label className="block text-gray-100 font-medium">Proof of Address</label>
                  <div className="group relative">
                    <div
                      className={cn(
                        "border-2 border-dashed border-white/20 rounded-xl p-8",
                  ...
                  {proofOfAddressImage.url && (
                    <div className="mt-4">
                      <p className="text-sm text-gray-400 mb-2">Proof of Address Preview:</p>
                      <img
        ...
        <CardFooter className="border-t border-white/20 p-8">
          <div className="space-y-3">
            ...
            <p className="text-gray-400/90 leading-relaxed">We will review your documents and update you soon.</p>
          </div>
        </CardFooter>
      </motion.div>
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}>
        <DialogContent className="bg-gray-900 border-white/20 rounded-2xl backdrop-blur-lg">
          <motion.div className="flex flex-col items-center p-8" initial={{ scale: 0.95 }} animate={{ scale: 1 }}>
            <div className="bg-green-500/20 p-6 rounded-2xl mb-6">
              <CheckCircle2 className="w-16 h-16 text-green-400" />
            </div>
            <DialogTitle className="text-3xl font-bold text-green-400 mb-4">KYC Submitted Successfully</DialogTitle>
            <Button
